...
void memo_method(int i, int j) {
    ...
    if (j == 0) {
        ...
        dp_max[i][1] = max(max(dp_max[i - 1][0], dp_max[i - 1][1]), dp_max[i - 1][2]) + nums[i][1];
        dp_min[i][1] = min(min(dp_min[i - 1][0], dp_min[i - 1][1]), dp_min[i - 1][2]) + nums[i][1];
    } else {
...
void tabulate_method(int n) {
    for (int i = 1; i <= n; i++) {
        ...
        dp_max[curr][1] = max(max(dp_max[!curr][0], dp_max[!curr][1]), dp_max[!curr][2]) + num2;
        dp_max[curr][2] = max(dp_max[!curr][1], dp_max[!curr][2]) + num3;

        dp_min[curr][0] = min(dp_min[!curr][0], dp_min[!curr][1]) + num1;
        dp_min[curr][1] = min(min(dp_min[!curr][0], dp_min[!curr][1]), dp_min[!curr][2]) + num2;
...
int main() {
    ...
    printf("%d %d", max(max(dp_max[N][0], dp_max[N][1]), dp_max[N][2]),
           min(min(dp_min[N][0], dp_min[N][1]),
               dp_min[N][2]));
#else
    printf("Tabulation\n");
    tabulate_method(N);
    printf("%d %d", max(max(dp_max[N % 2][0], dp_max[N % 2][1]), dp_max[N % 2][2]),
           min(min(dp_min[N % 2][0], dp_min[N % 2][1]),
               dp_min[N % 2][2]));
#endif
    return 0;
}