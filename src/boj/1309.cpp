...
// Comment: Memoization may not succeed in your environment when input size is very big due to stack size constraint
//
...
int memo_method(int i, int j) {
    ...
    for (int k = 0; k < 3; k++) {
        if (j != 0 && j == k) continue;
        dp[i][j] = ((dp[i][j] % DIV_MOD) + (memo_method(i - 1, k) % DIV_MOD)) % DIV_MOD;
    }
    filled[i][j] = true;
    return dp[i][j];
}
...
int tabulate_method(int n) {
    ...
    for (int i = 2; i <= n; i++) {
        dp[i][0] = ((dp[i - 1][0] % DIV_MOD) + (dp[i - 1][1] % DIV_MOD) + (dp[i - 1][2] % DIV_MOD)) % DIV_MOD;
        dp[i][1] = ((dp[i - 1][0] % DIV_MOD) + (dp[i - 1][2] % DIV_MOD)) % DIV_MOD;
        dp[i][2] = ((dp[i - 1][0] % DIV_MOD) + (dp[i - 1][1] % DIV_MOD)) % DIV_MOD;
    }

    return ((dp[n][0] % DIV_MOD) + (dp[n][1] % DIV_MOD) + (dp[n][2] % DIV_MOD)) % DIV_MOD;
}
#endif
